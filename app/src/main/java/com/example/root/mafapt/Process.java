    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnAnalyse.setOnClickListener(new View.OnClickListener() {
            ...
            try {
                        ...
                        if (supports.get(i1-1)[0] == i) { // if jointNo. == current joint i
                            ...
                            for (int i2 = 1; i2 <= NCJT; i2++) {
                                ...
                                if (supports.get(i1-1)[i2+1-1] == 1) {
                                    K = K + 1;
                                    NSC[i3-1] = K;
                                    //Toast.makeText(Process.this, "NSC at position " + i3 + " = " + K, Toast.LENGTH_LONG).show();
                                } else {
                                    J = J + 1;
                                    NSC[i3-1] = J;
                                }
                ...
                StringBuilder forces = new StringBuilder();
                for (int im = 0; im < members.size(); im++) {
                    JB = members.get(im)[0];
                    JE = members.get(im)[1];
                    I = members.get(im)[2];
                    ...
                    forces.append(Q[0]);
                    if(im < members.size() - 1) forces.append(" ");
                ...
                if (!out.toString().equals("")) {
                    ...
                    outputIntent.putExtra("forces", forces.toString());
        ...
        tlJointCoordinates.setGravity(Gravity.CENTER);
        tlSupports.setGravity(Gravity.CENTER);
        tlMaterialProperties.setGravity(Gravity.CENTER);
        tlCrossSectionProperties.setGravity(Gravity.CENTER);
        tlMembers.setGravity(Gravity.CENTER);
        tlJointLoads.setGravity(Gravity.CENTER);
        ...
        tempTV1.setTypeface(null, Typeface.BOLD);
        tempTV2.setTypeface(null, Typeface.BOLD);
        tempTV3.setTypeface(null, Typeface.BOLD);
        tempTV4.setTypeface(null, Typeface.BOLD);
